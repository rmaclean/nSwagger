...
namespace nSwagger
{
    ...
    public class SwaggerPetstore
    {
        ...
        {
            {
                var response = await HTTP.HTTP.PostAsync(new Uri(url + "/pets", UriKind.Absolute), new HTTPOptions(TimeSpan.FromSeconds(30)), new StringContent(JsonConvert.SerializeObject(pet)));
                if (response == null)
                {
                    return new APIResponse<Pet>(false);
                }
        ...
        {
            {
                var response = await HTTP.HTTP.DeleteAsync(new Uri(url + "/pets/" + id + "", UriKind.Absolute), new HTTPOptions(TimeSpan.FromSeconds(30)));
                if (response == null)
                {
        ...
        {
            {
                var response = await HTTP.HTTP.GetAsync(new Uri(url + "/pets/" + id + "", UriKind.Absolute), new HTTPOptions(TimeSpan.FromSeconds(30)));
                if (response == null)
                {
                    return new APIResponse<Pet>(false);
                }
